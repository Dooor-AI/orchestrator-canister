...
export default class DooorCanister {
  ...
  // ===== LLM =====
  @update([], IDL.Text) async getAllModels(): Promise<string> {
    return await this.llmService.getAllModels();
  }
  @update([IDL.Text], IDL.Text) async getModelById(modelId: string): Promise<string> {
    return await this.llmService.getModelById(modelId);
  }
  @update([IDL.Text], IDL.Text) async setDefaultModel(modelId: string): Promise<string> {
    return await this.llmService.setDefaultModel(modelId);
  }

  // ===== TEE =====
  @update([], IDL.Text) async validateTeeInfrastructure(): Promise<string> {
